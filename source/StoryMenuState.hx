...
class StoryMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		scoreText = new FlxText(10, 650, 0, "SCORE: 49324858", 36);
		...
		var lockthingy = Paths.getSparrowAtlas('campaign_menu_UI_assets');
		...
		left = new FlxSprite(0, -500).loadGraphic(Paths.image('left'));
		...
		right = new FlxSprite(-10, -500).loadGraphic(Paths.image('right'));
		// left.scrollFactor.set(0, yScroll);
		...
		story = new FlxSprite(40, 50); // originally -350, 200
		story.frames = Paths.getSparrowAtlas('mainmenu/menu_story_mode');
		story.antialiasing = ClientPrefs.globalAntialiasing;
		story.animation.addByPrefix('story_mode white', 'story_mode white', 19);
		story.animation.play('story_mode white');
		...
		var free = new FlxSprite().loadGraphic(Paths.image('storymodebg'));
		free.screenCenter();
		// add(free);
	...
	override function update(elapsed:Float)
	{
		...
		if (!movedBack && !selectedWeek)
		{
			...
			if (upP)
			{
				...
				// FlxTween.tween(bgSprite, {x: -1000}, 1, {ease: FlxEase.expoIn});
			}
			...
			else if (controls.ACCEPT)
			{
				...
				FlxTween.tween(bgSprite, {x: 1000}, 1, {ease: FlxEase.expoIn});
				FlxTween.tween(right, {x: 1000}, 1.4, {ease: FlxEase.expoIn});
				FlxTween.tween(left, {x: 10000}, 1.2, {ease: FlxEase.expoIn});
				FlxTween.tween(story, {y: 1000}, 1.0, {ease: FlxEase.expoIn});
			}
		}
